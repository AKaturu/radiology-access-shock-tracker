...
def main() -> None:
    ...
    if "fetch-fda-mqsa" in steps:
        ...
        _run("Fetch FDA MQSA", radshock + [
            "fetch-fda-mqsa",
            "--output-dir", str(fda_archive_dir),
            "--force",
        ])
    ...
    if "prepare-mqsa-review" in steps:
        ...
        if not mqsa_archive.exists():
            ...
            if candidates:
                ...
                skip = {"prepare-mqsa-review", "geocode-mqsa-review",
                        "finalize-mqsa-review", "ingest-snapshot"}
                steps -= skip
                return
        _run("Prepare MQSA review", radshock + [
            "prepare-mqsa-review", str(mqsa_archive),
            "--output-csv", str(mqsa_review_csv),
            "--state", state,
            "--force",
        ])
    ...
    if "geocode-mqsa-review" in steps and mqsa_review_csv.exists():
        _run("Geocode MQSA review", radshock + [
            "geocode-mqsa-review", str(mqsa_review_csv),
            "--output-csv", str(mqsa_geocoded_csv),
            "--provider", "census",
    ...
    if "finalize-mqsa-review" in steps:
        ...
        if not review_input.exists():
            ...
            _run("Finalize MQSA review", radshock + [
                "finalize-mqsa-review", str(review_input),
                "--output-csv", str(mqsa_final_csv),
                "--force",
            ])
    ...
    if "ingest-snapshot" in steps and mqsa_final_csv.exists():
        _run("Ingest MQSA snapshot", radshock + [
            "ingest-snapshot", str(mqsa_final_csv),
            "--as-of", snapshot_date,
    ...
    if "prepare-hrsa-review" in steps:
        ...
        if hrsa_input and Path(hrsa_input).exists():
            _run("Prepare HRSA candidate review", radshock + [
                "prepare-hrsa-candidate-review", str(hrsa_input),
                "--output-csv", str(hrsa_review_csv),
                "--state", state,
                "--force",
            ])
            ...
            _run("Prepare county-centroid candidate review (HRSA CSV not provided)", radshock + [
                "prepare-candidate-review",
                "--counties-csv", str(work_dir / f"counties_{state}.csv"),
    ...
    if "finalize-hrsa-review" in steps and hrsa_review_csv.exists():
        _run("Finalize HRSA candidate review", radshock + [
            "finalize-candidate-review", str(hrsa_review_csv),
            "--output-csv", str(hrsa_final_csv),
            "--force",
        ])
    ...
    if "prepare-travel-time" in steps:
        ...
        if pop_csv.exists() and fac_csv.exists():
            _run("Prepare travel-time review", radshock + [
                "prepare-travel-time-review",
                "--population-csv", str(pop_csv),
                "--facilities-csv", str(fac_csv),
                "--output-csv", str(travel_time_review_csv),
                "--force",
            ])
    ...
    if "finalize-travel-time" in steps and travel_time_review_csv.exists():
        _run("Finalize travel-time review", radshock + [
            "finalize-travel-time-review", str(travel_time_review_csv),
            "--output-csv", str(travel_time_matrix_csv),
            "--force",
        ])
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hrsa-csv", type=Path,
                        help="HRSA Health Center Sites CSV input.")
    parser.add_argument("--population-csv", type=Path,
                        help="Population points CSV (for travel-time prep).")
    parser.add_argument("--facilities-csv", type=Path,
