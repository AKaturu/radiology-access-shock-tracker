...
from radshock.data_quality import audit_csv_quality
...
def test_csv_quality_audit_flags_invalid_numeric_and_duplicate_key(tmp_path) -> None:
    source = tmp_path / "facilities.csv"
    pd.DataFrame(
        [
            ["F1", "Facility", "bad-lat", "-78.0", "", "true"],
            ["F1", "Facility duplicate", "35.0", "-78.1", "", "true"],
        ],
        columns=[
            "facility_id",
            "facility_name",
    ...
    audit = audit_csv_quality(source, dataset_type="facilities")
    ...
    assert checks["numeric_values"]["status"] == "FAIL"
